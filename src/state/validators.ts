...
export const checkFolderNameUnique = (
  ...
  const siblings = parent.children
    .map((id: string) => state.nodes[id])
    .filter((node: FSNode) => node && node.type === "folder" && node.id !== excludeId);

  const duplicate = siblings.find(
    (sibling: FSNode) => sibling.name.trim().toLowerCase() === trimmed
  );
...
export const checkFileNameUnique = (
  ...
  const siblings = parent.children
    .map((id: string) => state.nodes[id])
    .filter(
      (node: FSNode) => node && node.type === "file" && node.id !== excludeId
    ) as any[];
  ...
  if (duplicate) {
    return {
      type: "duplicate",
      message: `${name.trim()}.${ext.trim()} already exists`,
    };
  }
