...
import { validateFolderCreation, validateFileCreation, validateFileRename } from './validators';
  ...
  switch (action.type) {
    ...
    case 'ADD_FOLDER': {
      ...
      if (validationError) {
        return {
          ...state,
          toasts: [...state.toasts, createToast(validationError.message, 'error')]
        };
      }
      ...
      return {
        ...updatedState,
        toasts: [...state.toasts, createToast(`Folder "${trimmedName}" created successfully`, 'success')]
      };
    }
    ...
    case 'ADD_FILE': {
      ...
      return {
        ...updatedState,
        toasts: [...state.toasts, createToast(`File "${trimmedName}.${trimmedExt}" created successfully`, 'success')]
      };
    }
    ...
    case 'RENAME_FILE': {
      ...
      // Validate
      const validationError = validateFileRename(state, fileId, newName, newExt);
      if (validationError) {
        return {
          ...state,
          toasts: [...state.toasts, createToast(validationError.message, 'error')]
      ...
      return {
        ...newState,
        toasts: [...state.toasts, createToast(`File renamed to "${trimmedName}.${trimmedExt}" successfully`, 'success')]
      };
    }
    ...
    case 'DELETE_NODE': {
      ...
      if (nodeId === state.rootId) {
        return {
          ...state,
          toasts: [...state.toasts, createToast('Cannot delete root folder', 'error')]
        };
      }
      ...
      const nodeDescription = node.type === 'file' 
        ? `"${node.name}.${(node as any).ext}"` 
        : `folder "${node.name}"`;
      
      return {
        ...updatedState,
        toasts: [...state.toasts, createToast(`${nodeDescription} deleted successfully`, 'success')]
      };
    }
